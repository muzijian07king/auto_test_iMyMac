...
class SupportPage(WebPage):
    ...
    def return_succeed_search(self, search):
        """判断搜索结果是否成功"""
        print(type(self.element_txet(support['search-content']).lower()))
        return search in self.element_txet(support['search-content']).lower() and search == self.element_txet(support['search'
                                                                                                              '-text'])
