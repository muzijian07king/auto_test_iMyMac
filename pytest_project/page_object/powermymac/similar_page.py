...
class SimilarPage(WebPage):
    ...
    def click_carousel(self):
        """点击评论轮播图下的索引按钮"""
        self.is_click(similar['carousel-li'])

    def return_carousel_index(self):
        """获取当前评论轮播图的索引"""
        with allure.step('判断评论轮播图是否切换成功'):
            return self.getAttribute(similar['carousel-inner'], 'class') == 'item active'

    def is_menu(self):
        """判断滑动到中间位置后menu是否出现"""
        with allure.step('判断滑动到中间位置后menu是否出现'):
            return self.getAttribute(similar['fix-menu'], 'class') == 'fixMenu fixIt'

    def is_goto_buy(self):
        """判断跳转购买页面内容与实际相同"""
        with allure.step("判断跳转购买页面内容与实际相同"):
            return self.find_element(similar['buy-month']) is not None
    ...
    def click_article_link(self, link):
        """点击文章链接"""
        self.is_click(similar[link])

    def is_goto_article(self, link):
        """判断是否到达文章页面"""
        with allure.step('判断是否到达文章页面'):
            return self.find_element(similar['article-body']) is not None and link in self.get_current_url()

    @allure.step('切换The Andvantage of Browser Cleanup轮播图')
    def click_carousel_indicators(self):
        """Key Features Similar Image Finder轮播图的切换"""
        self.is_click(similar['carousel-indicators-li'])

    def return_carousel_indicators_index(self):
        """获取当前Key Features Similar Image Finder轮播图的索引"""
        with allure.step('判断轮播图是否切换成功'):
            return self.getAttribute(similar['carousel-indicators-inner'], 'class') == 'item active'
