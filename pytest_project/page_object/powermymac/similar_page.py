...
class SimilarPage(WebPage):
    ...
    def download_container_ppm(self):
        """点击下载按钮"""
        self.is_click(similar['container-download'], 1)

    @allure.step('点击container栏的购买')
    def goto_container_buy(self):
        """去购买页面"""
        self.is_click(similar['container-buy'])

    @allure.step('移动到技巧栏')
    def scroll_tip(self):
        self.scroll_to_loc(similar['tip'])

    def click_link(self, index: int, name):
        """点击技巧文章"""
        with allure.step(f'点击技巧文章链接:{name}'):
            self.is_click(similar.readYaml(f'$.tips.tip-{index}'))
    ...
    def download_summary_ppm(self):
        """点击下载按钮"""
        self.is_click(similar['summary-download'], 1)

    @allure.step('点击summary栏的购买')
    def goto_summary_buy(self):
        """去购买页面"""
        self.is_click(similar['summary-buy'])
    ...
    def download_navbar_ppm(self):
        """点击下载按钮"""
        self.is_click(similar['navbar-download'], 1)

    @allure.step('点击navbar栏的购买')
    def goto_navbar_buy(self):
        """去购买页面"""
        self.is_click(similar['navbar-buy'])
