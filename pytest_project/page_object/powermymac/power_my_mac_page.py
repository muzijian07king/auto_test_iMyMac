...
class PMMPage(WebPage):
    ...
    def download_navbar_ppm(self):
        """点击下载按钮"""
        self.is_click(pmm['navbar-download'], 1)

    @allure.step('点击navbar栏的购买')
    def goto_navbar_buy(self):
        """去购买页面"""
        self.is_click(pmm['navbar-buy'])
    ...
    def assert_popup_nav(self):
        self.allure_assert('判断是否弹出导航栏', ('eq', self.is_display(pmm['navbar']), True))
