...
cleaner = Element('PowerMyMac/common')
...
class CleanerPage(WebPage):
    ...
    def download_container_ppm(self):
        """点击下载按钮"""
        self.is_click(cleaner['container-download'], 1)

    @allure.step('点击container栏的购买')
    def goto_container_buy(self):
        """去购买页面"""
        self.is_click(cleaner['container-buy'])

    @allure.step('移动到技巧栏')
    def scroll_tip(self):
        self.scroll_to_loc(cleaner['tip'])

    def click_link(self, index: int, name):
        """点击技巧文章"""
        with allure.step(f'点击技巧文章链接:{name}'):
            self.is_click(cleaner.readYaml(f'$.tips.tip-{index}'))
    ...
    def download_summary_ppm(self):
        """点击下载按钮"""
        self.is_click(cleaner['summary-download'], 1)

    @allure.step('点击summary栏的购买')
    def goto_summary_buy(self):
        """去购买页面"""
        self.is_click(cleaner['summary-buy'])
    ...
    def download_navbar_ppm(self):
        """点击下载按钮"""
        self.is_click(cleaner['navbar-download'], 1)

    @allure.step('点击navbar栏的购买')
    def goto_navbar_buy(self):
        """去购买页面"""
        self.is_click(cleaner['navbar-buy'])
    ...
    def assert_popup_nav(self):
        self.allure_assert('判断是否弹出导航栏', ('eq', self.is_display(cleaner['navbar']), True))
