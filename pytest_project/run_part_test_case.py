...
def get_test_part():
    parse = argparse.ArgumentParser(description='添加pytest参数')
    parse.add_argument('--pytest', default='', nargs='?')
...
def main(part):
    """主函数"""
    steps = [
        f"pytest {part} --alluredir report\\data --clean-alluredir" if WIN else f'pytest {part} --alluredir report/data'
                                                                                '--clean-alluredir',
